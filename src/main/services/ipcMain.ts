...
export default {
  Mainfunc() {
    ...
    //置顶
    ipcMain.on('window-top', (event) => {
      if (BrowserWindow.fromWebContents(event.sender)?.isAlwaysOnTop()){
        BrowserWindow.fromWebContents(event.sender)?.setAlwaysOnTop(false);
